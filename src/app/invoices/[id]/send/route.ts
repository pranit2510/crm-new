...
export async function POST (
  ...
  /* ── 1. fetch invoice + client in one round-trip ─────────────────── */
  ...
  /*  clients comes back as **array** when joined; normalise to 1 row  */
  ...
  /* ── 2. shoot the e-mail via Nodemailer (SMTP) ───────────────────── */
  try {
    const transporter = nodemailer.createTransport({
      host  : process.env.SMTP_HOST,
      port  : Number(process.env.SMTP_PORT),
      secure: Number(process.env.SMTP_PORT) === 465,      // TLS if 465
